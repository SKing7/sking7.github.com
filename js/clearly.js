...
window.__getMyClearlyResults = function () {
    ...
    // debug
    MyClearly.log = function (message) {
    ...
    // text length,
    MyClearly.getTextLength = function (text) {
        return text.replace(/[\s\n\r]+/gi, '').length;
    };

    // word count,
    MyClearly.getWordCount = function (text) {
        var _text = text;
        // do stuff
    ...
    // content
    MyClearly.getContent = function () {
        this.detectLanguage();
        return this.find();
    };

    // Detect language by randomly selected paragraphs from page
    MyClearly.detectLanguage = function () {
    ...
    MyClearly.getAbsoluteUrl = function (url) {
    ...
    // _return__links.push({href: MyClearly.getAbsoluteUrl(_href), text: _node.textContent});
    // _return__images.push({type: 'large', src: MyClearly.getAbsoluteUrl(_node.src), width: _width, height: _height});
    // _return__images.push({type: 'medium', src: MyClearly.getAbsoluteUrl(_node.src), width: _width, height: _height});

    // Decide tagName of a node, we just need #text, a, img nodes
    MyClearly.getContent_detectNodeTagName = function (_node) {
        var _tag_name;
    ...
    MyClearly.exploreNodeAndGetStuff = function (_nodeToExplore, _justExploring) {
        ...
        var _recursive = function (_node) {
            ...
            var _tag_name = MyClearly.getContent_detectNodeTagName(_node),
                _result = {
            ...
            case ((_tag_name === '#text')):
                ...
                // result
                _result._length__plain_text = MyClearly.getTextLength(_nodeText);
                _result._count__plain_words = MyClearly.getWordCount(_nodeText);
            ...
            if (_result._is__container || ((_result.__index === 1) && (_justExploring === true))) {
                ...
                if (!_justExploring) {
                    switch (true) {
                    case ((MyClearly.language !== 'cjk') && ((_result._count__links * 2) >= _result._count__plain_words)):
                        /* link ratio */
                    case ((MyClearly.language !== 'cjk') && (_result._length__plain_text < (65 / 3))):
                        /* text length */
                    case ((MyClearly.language !== 'cjk') && (_result._count__plain_words < 5)):
                        /* words */
                    case ((MyClearly.language === 'cjk') && (_result._length__plain_text < 10)):
                        /* text length */
                    case ((MyClearly.language === 'cjk') && (_result._count__plain_words < 2)):
                        /* words */
                        break;
                    default:
                        // good candidate
    ...
    MyClearly.processCandidates = function (_candidatesToProcess) {
        // process this var
        ...
        for (var i = 0, _i = _candidates.length; i < _i; i++) {
            ...
            // candidate details
            _candidates[i]['__candidate_details'] = MyClearly.computeDetailsForCandidate(_candidates[i], _main);
            ...
            // points
            _candidates[i].__points_history = MyClearly.computePointsForCandidate(_candidates[i], _main);
    ...
    MyClearly.computeDetailsForCandidate = function (_e, _main) {
        var _r = {};
    ...
    MyClearly.computePointsForCandidate = function (_e, _main) {
    ...
    MyClearly.processCandidatesSecond = function (_processedCandidates) {
    ...
    MyClearly.computeDetailsForCandidateSecond = function (_e, _main) {
        var _r = {};
    ...
    MyClearly.computePointsForCandidateSecond = function (_e) {
        var _details = _e.__candidate_details,
    ...
    MyClearly.computePointsForCandidateThird = function (_e) {
        var _details = _e.__candidate_details,
            _details_second = _e.__candidate_details_second,
            _points_history = [];
    ...
    MyClearly.computePointsForCandidate__do = function (_ratio_remaining, _power, _ratio, _points_history) {
    ...
    MyClearly.buildHTMLForNode = function (_nodeToBuildHTMLFor, _custom_mode) {
        var _global__element_index = 0,
            _global__the_html = '',
        ...
        var _recursive = function (_node) {
            ...
            // vars
            var _explored = false,
                _tag_name = MyClearly.getContent_detectNodeTagName(_node),
            ...
            // skipped link
            if (_tag_name === 'a') {
                _explored = (_explored || MyClearly.exploreNodeAndGetStuff(_node, true));
                switch (true) {
            ...
            if (parsingOptions._elements_link_density.indexOf('|' + _tag_name + '|') > -1) {
                ...
                case (MyClearly.language === 'cjk' && (_explored._length__plain_text > (65 * 3 * 1))):
            ...
            if (_tag_name === 'img') {
                ...
                case (_explored._is__image_large):
                    MyClearly.images.push({src: MyClearly.getAbsoluteUrl(_node.src), width: _$node.width(), height: _$node.height()});
            ...
            if (_tag_name === 'a') {
                _explored = (_explored || MyClearly.exploreNodeAndGetStuff(_node, true));
                _$node = $(_node);
                MyClearly.links.push({href: MyClearly.getAbsoluteUrl(_node.href), text: _$node.text()});
                switch (true) {
            ...
            default:
                ...
                _contentsLength = MyClearly.getTextLength(_contents);
                switch (true) {
            ...
            if (parsingOptions._elements_link_density.indexOf('|' + _tag_name + '|') > -1) {
                ...
                case (!((MyClearly.language === 'cjk') && (_contentsLength / _initialLength) < 0.1)):
                case ((_global__exploreNodeToBuildHTMLFor && ((_explored._length__plain_text / _global__exploreNodeToBuildHTMLFor._length__plain_text) > 0.25))):
                case ((MyClearly.language === 'cjk') && (_global__exploreNodeToBuildHTMLFor && ((_explored._length__plain_text / _global__exploreNodeToBuildHTMLFor._length__plain_text) > 0.1))):
                    break;
                default:
                    _global__the_html = _global__the_html.substr(0, _pos__start__before);
                    return;
    ...
    MyClearly.find = function () {
        ...
        (function () {
            while (true) {
                ...
                if (_prevNode.previousSibling) {
                    ...
                    // get html
                    var _h = MyClearly.buildHTMLForNode(_prevNode, 'above-the-target');
                    _prevHTML = _h + _prevHTML;
                    _foundHTML = _h + _foundHTML;

                    // finished?
                    if (MyClearly.getTextLength(_prevHTML.replace(/<[^>]+?>/gi, '')) > (65 * 3 * 3)) {
                        return;
                    }
                    ...
                    if (_headingStartPos > -1) {
                        var _toHeadingLength = MyClearly.getTextLength(_foundHTML.substr(0, _headingStartPos).replace(/<[^>]+?>/gi, ''));
                        if (_toHeadingLength < (65 * 3 * 2)) {
                            MyClearly.log('use title found in page');
                            _foundTitle = true;
        ...
        if (!_foundTitle) {
            if (document.title > '') {
                MyClearly.log('use title from document.title');
                var _the_title = '',
    ...
    MyClearly.findInPage = function (_pageWindow) {
